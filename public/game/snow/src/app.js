...
export class SnowApp {
  ...
  _update(dt) {
    ...
    if (h.alive && !h.crafting) {
      let f = 0, r = 0;
      if (this.keys['w']) f += 1; if (this.keys['s']) f -= 1;
      if (this.keys['d']) r += 1; if (this.keys['a']) r -= 1;
      h.cover = !!this.keys['c'];
      if (f || r) {
  ...
  _drawBillboard(ctx, it, now) {
    ...
    if (it.kind === 'player') {
      ...
      const r = Math.max(2, (5 / it.depth) * this.focal);
      ctx.fillStyle = `rgba(255,255,255,${alpha})`;
      ctx.strokeStyle = `rgba(160,195,220,${alpha})`;
